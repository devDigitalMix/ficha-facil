...
def main():
    ...
    # Guarda: uma magia cujo nome não casa com a entrada do capítulo 7 nunca tem
    # corpo extraído — e então NADA aqui a confere, calada. Foi o que aconteceu com
    # quatro magias (Bênção, Pele-Casca, Invocar Morto-Vivo, Proteção Contra
    # Energia): a grafia da lista da classe estava ganhando da entrada que define a
    # magia. Isso é erro, não achado para conferir à mão.
    sem_corpo = sorted(i['nome'] for i in mg['itens'] if i['nome'] not in corpos)
    if sem_corpo:
        print('ERRO: %d magia(s) sem entrada no capítulo 7 — o nome do catálogo não '
              'casa com o do livro, e a paráfrase não é conferida por ninguém:'
              % len(sem_corpo))
        for n in sem_corpo:
            print('  ' + n)
        return 1
